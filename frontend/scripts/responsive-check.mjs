...
const run = async () => {
  ...
  await seedPage.evaluate((items) => {
    ...
    // Fake auth so /profile renders instead of redirecting to /login.
    localStorage.setItem('shopease_token', 'fake-test-token');
    localStorage.setItem('shopease_user_first_name', 'Test');
    localStorage.setItem('shopease_user_email', 'test@example.com');
  }, SEED_CART);
